...
class UnlockCommand extends SimpleCommand {
    ...
    async trigger(message: Message, args: carg[], lang: language, gInfo: GuildModel, tcInfo: tcSchema, admin: boolean): Promise<void | Error> {
        ...
        if (!message.member?.voiceState.channelID) {
            message.channel.createMessage(lang.commands.guild.unlock.notInVoiceChannel);
            return;
        }
        ...
        if (!voiceChannel.parentID) {
            message.channel.createMessage(lang.commands.guild.unlock.notEnabled);
            return;
        }
        ...
        if (voiceChannel.userLimit != 1) {
            message.channel.createMessage(lang.commands.guild.unlock.alreadyUnlocked);
            return;
        } else {
            // Do lock
            await voiceChannel.edit({userLimit: catInfo.channelUserLimit});
            message.channel.createMessage(lang.commands.guild.unlock.success);
